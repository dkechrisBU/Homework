...
%{
mu_init = [3 3; -4 -1; 2 -4]
gall_WCSS = kmeansimp(3,mu_init,gall)
%}
%% 3.2b

%{
mu_alt_init = [-0.14 2.61; 3.15 0.84; -3.28 -1.58]
gall_alt_WCSS = kmeansimp(3,mu_alt_init,gall)
% the three clusters should all reduce to the same centers regardles of
initialization for k=3
% this makes sense since they are far enough apart to be differentiable
%}
%% 3.2c
...
mu_best = zeros(3,2)
WCSS_best = 10000
% k can be changes to get the different graphs
k = 10
for i = 1:k:k*10
    mu_init = rxy(i:i+k-1,:);
    j = 1+(i-1)/k;
...
mu_best
WCSS_best
result = kmeansimp(k,mu_best,gall);

% Lowest WCSS
% j = 8
% WCSS_best = 13.2833
% mu_best = [1.1555 1.1145; 1.1809 0.3577; 1.4321 1.1785]
kmeansimp(3,mu_best,gall)

%}
%% 3.2d
%the WCSS keeps decreasing as k increases, but there is a clear elbow at
%k=3
%{
k_WCSS = zeros(10)
...
%create 100 random points between 0 and 40
rx = (40)*rand(100,1)
ry = (40)*rand(100,1)
rxy = [rx ry]
...
for i = 1:k:k*10
    mu_init = rxy(i:i+k-1,:);
    j = 1+(i-1)/k;
    output = kmeansimp(k,mu_init,mppg);
    nba_WCSS(j) = output;
    if output < nba_WCSS_best
        nba_WCSS_best = output;
        k_mu_best = mu_init
        nba_j_best = j;
    end
end
...
nba_WCSS
nba_WCSS_best
%nba_WCSS_best = [0  418.0056   13.2883   11.2170    9.7814    9.4245
% 7.9506    6.2270    7.6788    8.2860]
nba_WCSS_best = min(nba_WCSS)
k_mu_best
nba_j_best
kmeansimp(k,k_mu_best,mppg);
%}
%% 3.2f 
% knn is incapable of creating valuable clusters for this circle pattern
% there is a relationship between the 3 rings, but a simple distance
% function with centroids is unable to determine what that relationship is
%{
% Problem 3.2(f): Generate Concentric Rings Dataset using
% sample_circle.m provided to you in the HW 3 folder on Blackboard.

circlepoints = sample_circle(3)
scatter(circlepoints(:,1),circlepoints(:,2))

k = 3

rx = -4 + (4+4)*rand(k*10,1)
ry = -4 + (4+4)*rand(k*10,1)
rxy = [rx ry]
r_WCSS = zeros(10,1)
mu_best = zeros(3,2)
WCSS_best = 10000
...
for i = 1:k:k*10
    mu_init = rxy(i:i+k-1,:);
    j = 1+(i-1)/k;
    output = kmeansimp(k,mu_init,circlepoints);
    r_WCSS(j) = output;
    if output < WCSS_best
        WCSS_best = output;
        mu_best = mu_init
...
mu_best
WCSS_best
result = kmeansimp(k,mu_best,circlepoints);
%}
